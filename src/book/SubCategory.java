...
Database 에서는 현실의 사물을 Entity라는 객체개념으로 표현한다.
...
이때, 하나의 레코드는 결국 하나의 객체로 봐야한다 .
 ...
 결론) 테이블에 존재하는 상품 레코드 수가 총 5개라면 , 
 개발자는 이 각각이 레코드를 5개의 인스턴스로 각각 받으면 된다.
 
 아래의 클래스는 로직 작성용이 아니다. 즉 한건의 레코드를 담기위한 저장공간용도로 사용할 클래스이다.
어플리케이션 설계분야에서 이러한 목적의 클래스를 가리켜 VO, DTO 라 한다
Value Object = 값만 담는 객체
...
//로직이 없는 클래스! Dummy클래스
//오직 데이터 담는 그릇용 (그래서 보호해야하니까 private)
//배열과 객체는 차원이 다른 방식으로 개발되는데, 객체로 처리하는 것이 작업방식이나 개념상 훨씬 더 좋다 
